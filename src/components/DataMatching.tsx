...
const DataMatching: React.FC<DataMatchingProps> = ({
  ...
  const [localSelectedDataset, setLocalSelectedDataset] = useState(selectedDataset);
  ...
  const findMatches = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/projects/${projectId}/match-data`, {
        method: 'POST',
  ...
  const handleSelectMatch = (index: number, match: DataMatch) => {
    setMatchResults(prev => prev.map((result, i) => 
      i === index 
  ...
  const getMatchIcon = (matchType: string) => {
    switch (matchType) {
      case 'exact': return <Check className="w-4 h-4 text-green-600" />;
      case 'serial_only': return <AlertTriangle className="w-4 h-4 text-yellow-600" />;
      case 'tag_only': return <AlertTriangle className="w-4 h-4 text-orange-600" />;
      case 'partial': return <AlertTriangle className="w-4 h-4 text-red-600" />;
  ...
  const unmatchedCount = matchResults.filter(r => r.matches.length === 0).length;
  ...
  return (
    <Box>
      ...
      {datasets.length > 0 && (
        ...
        >
          <Group justify="space-between" mb="sm">
            ...
            >
              {localSelectedDataset ? `${localSelectedDataset.name} (${localSelectedDataset.totalRows} rows)` : 'No dataset selected'}
            </Text>
          </Group>
          <Select
          ...
          {localSelectedDataset && (
            <Box>
              ...
              >
              <strong>Description:</strong> {localSelectedDataset.description || 'No description'}
              </Text>
              <Text 
                size="sm" 
                style={{ 
              ...
              >
              <strong>Files:</strong> {localSelectedDataset.files?.join(', ') || 'No files'}
              </Text>
            </Box>
          )}
        </Paper>
          ...
          .map((result, index) => (
            ...
            >
              <Group justify="space-between" mb="sm">
                ...
                >
                {result.extractedItem.Item_Description} | {result.extractedItem.Serial_Number}
                </Text>
              </Group>
            ...
            {/* Extracted Data */}
              ...
              >
                ...
                <SimpleGrid cols={2} spacing="xs">
                  <Text size="sm" style={{ fontFamily: 'Inter Tight, sans-serif' }}>
                    <strong>Item:</strong> {result.extractedItem.Item_Description}
                  </Text>
                  <Text size="sm" style={{ fontFamily: 'Inter Tight, sans-serif' }}>
                    <strong>Serial:</strong> {result.extractedItem.Serial_Number}
                  </Text>
                  <Text size="sm" style={{ fontFamily: 'Inter Tight, sans-serif' }}>
            ...
            {result.matches.length > 0 ? (
                ...
                {result.matches.slice(0, 3).map((match, matchIndex) => (
                    ...
                    key={matchIndex}
                      style={{
                        border: result.selectedMatch?.item.id === match.item.id 
                          ? '2px solid #176B87' 
                          : '1px solid #86B6F6',
                        backgroundColor: result.selectedMatch?.item.id === match.item.id 
                          ? '#EEF5FF' 
                          : 'white',
                        borderRadius: '8px',
                        cursor: 'pointer'
                  ...
                  >
                      <Group justify="space-between" mb="sm">
                        ...
                        {getMatchIcon(match.match_type)}
                          <Badge 
                            color={
                              match.confidence >= 90 ? 'green' :
                              match.confidence >= 70 ? 'yellow' :
                              match.confidence >= 50 ? 'orange' : 'red'
                            }
                            variant="light"
                            size="sm"
                          >
                      ...
                      {result.selectedMatch?.item.id === match.item.id && (
                          <CheckCircle size={16} style={{ color: '#176B87' }} />
                        )}
                      </Group>
                      <SimpleGrid cols={2} spacing="xs">
                        <Text size="sm" style={{ fontFamily: 'Inter Tight, sans-serif' }}>
      ...
      {/* Action Buttons */}
      <Group justify="flex-end" gap="sm" mt="lg" pt="md" style={{ borderTop: '1px solid #86B6F6' }}>
        <Button
          onClick={() => setMatchResults(prev => prev.map(r => ({ ...r, selectedMatch: undefined, isCorrected: false })))}
          variant="outline"
          style={{
            color: '#176B87',
            borderColor: '#86B6F6',
