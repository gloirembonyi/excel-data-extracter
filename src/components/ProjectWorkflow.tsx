...
import { X, Upload, Database, FileText, Download, CheckCircle, Search } from 'lucide-react';
...
const ProjectWorkflow: React.FC<ProjectWorkflowProps> = ({
  ...
  const [activeStep, setActiveStep] = useState<'upload' | 'extract' | 'match' | 'export'>('upload');
  ...
  const handleFindInDataset = async () => {
    ...
    try {
      ...
      for (const dataset of selectedDatasets) {
        const response = await fetch(`http://localhost:8000/datasets/${dataset.id}`);
        ...
        const matches = extractedData.map(extractedItem => {
          ...
          const matchingItems = datasetItems.filter((datasetItem: any) => {
            ...
            if (exactMatch) {
              ...
              return datasetValue.toLowerCase().includes(searchValue.toLowerCase()) || 
          ...
          if (match.hasMatch) {
            match.matches.forEach((item: any, index: number) => {
              combinedTableData.push({
                ...
                ITEM: item.Item_Description || item.ITEM || item.item_description || 'N/A',
                CODE: item.Code || item.CODE || item.code || item.Tag_Number || item.tag_number || 'N/A',
                'SERIAL NUMBER': item.Serial_Number || item['SERIAL NUMBER'] || item.serial_number || 'N/A',
    ...
    <Stack gap="xl">
      ...
      >
        <Stack gap="lg">
          ...
          <Paper 
            style={{
              border: activeStep === 'upload' ? '2px solid #176B87' : '1px solid #86B6F6',
            ...
            <Group justify="space-between" mb="md">
              <Group gap="md">
                <Avatar 
                  size="md" 
                  style={{ 
                    backgroundColor: activeStep === 'upload' ? '#176B87' : '#B4D4FF',
                    color: 'white'
                  }}
                >
                  1
          ...
          <Paper 
            style={{
              border: activeStep === 'extract' ? '2px solid #176B87' : '1px solid #86B6F6',
            ...
            <Group justify="space-between" mb="md">
              <Group gap="md">
                <Avatar 
                  size="md" 
                  style={{ 
                    backgroundColor: activeStep === 'extract' ? '#176B87' : '#B4D4FF',
                    color: 'white'
                  }}
                >
                  2
          ...
          <Paper 
            style={{
              border: activeStep === 'match' ? '2px solid #176B87' : '1px solid #86B6F6',
            ...
            <Group justify="space-between" mb="md">
              <Group gap="md">
                <Avatar 
                  size="md" 
                  style={{ 
                    backgroundColor: activeStep === 'match' ? '#176B87' : '#B4D4FF',
                    color: 'white'
                  }}
                >
                  3
            ...
            {extractedData.length > 0 ? (
              <Stack gap="md">
                  ...
                  <Group justify="space-between" align="center" mb="md">
                    ...
                    {selectedDataset && (
                      <Menu shadow="md" width={200}>
                        ...
                        <Menu.Dropdown>
                          <Menu.Label style={{ fontFamily: 'Inter Tight, sans-serif' }}>
                            Search Options
                          </Menu.Label>
                          <Menu.Item 
                          ...
                          <Menu.Item 
                            onClick={() => {
                              ...
                              setExactMatch(false);
                              setSelectedDatasets(selectedDataset ? [selectedDataset] : []);
                              if (selectedDataset) {
                                handleFindInDataset();
                              }
                            }}
                ...
                {showMatchingResults && combinedResults.length > 0 && (
                  ...
                  >
                    <Group justify="space-between" align="center" mb="md">
                      ...
                      >
                        Search Results - Excel Table Format ({combinedResults.length} items found)
                      </Title>
                      <Group gap="sm">
                        <Button
                ...
                {showMatchingResults && matchingResults.length > 0 && (
                  ...
                  >
                    <Group justify="space-between" align="center" mb="md">
                      ...
                      >
                        Detailed Matching Results ({matchingResults.filter(r => r.hasMatch).length} matches found)
                      </Title>
                    </Group>
                    ...
                    <Stack gap="md">
                      {matchingResults.map((result, index) => (
                        <Paper 
                          key={index}
                          style={{
                            border: result.hasMatch ? '2px solid #4CAF50' : '1px solid #FFC107',
                            borderRadius: '8px',
                            padding: '12px',
                            backgroundColor: result.hasMatch ? '#F1F8E9' : '#FFF8E1'
                          }}
                        >
                          <Group justify="space-between" mb="sm">
                            ...
                            >
                              {result.searchField}: {result.searchValue || 'N/A'}
                            </Text>
                            <Badge 
                              ...
                              size="sm"
                            >
                              {result.hasMatch ? `${result.matches.length} matches` : 'No matches'}
                            </Badge>
                          </Group>
                          ...
                          {!result.hasMatch && (
                            ...
                            >
                              No matching {result.searchField} values found in dataset
                            </Text>
                          )}
                        </Paper>
                      ))}
                    ...
                    <Stack gap="md">
                      {imageResults.map((result, index) => (
                        ...
                        >
                          <Group justify="space-between" mb="sm">
                            ...
                            <Badge 
                              color={
                                ...
                                result.status === 'processing' ? 'blue' : 'gray'
                              }
                              variant="light"
                              size="sm"
                            >
                          ...
                          {result.status === 'completed' && result.tableData && result.tableData.length > 0 && (
                            <Box style={{ margin: 0, padding: 0 }}>
                              <ExcelLikeTable
                ...
                {(masterData.length > 0 || selectedDataset) ? (
                  ...
                  >
                    Upload a master dataset or select a dataset to enable data matching and correction.
                  </Alert>
                )}
              </Stack>
            ) : (
              ...
              >
                Process images first to extract data, then you can match it with master data.
              </Alert>
            )}
          </Paper>
          ...
          <Paper 
            style={{
              border: activeStep === 'export' ? '2px solid #176B87' : '1px solid #86B6F6',
            ...
            <Group justify="space-between" mb="md">
              <Group gap="md">
                <Avatar 
                  size="md" 
                  style={{ 
                    backgroundColor: activeStep === 'export' ? '#176B87' : '#B4D4FF',
                    color: 'white'
                  }}
                >
                  4
      ...
      >
        <Stack gap="lg">
          ...
          >
            Search for extracted data items in selected datasets by matching field values.
          </Text>
          ...
          <Select
            ...
            data={getAvailableFields().map(field => ({ value: field, label: field }))}
            required
            radius="lg"
            styles={{
              label: {
          ...
          >
            ...
            >
              • Searching by: <strong>{searchField}</strong><br/>
              • Match type: <strong>{exactMatch ? 'Exact' : 'Partial'}</strong><br/>
              • Selected datasets: <strong>{selectedDatasets.length}</strong> ({selectedDatasets.map(d => d.name).join(', ')})<br/>
              • Items to search: <strong>{extractedData.length}</strong>
            </Text>
          </Paper>
        </Stack>
