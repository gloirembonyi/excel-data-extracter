...
import { Plus, Folder, FileSpreadsheet, Trash2, Download, Upload, Eye, EyeOff } from 'lucide-react';
...
const ProjectManager: React.FC<ProjectManagerProps> = ({
    ...
    const exportSelectedTables = () => {
        ...
        const selectedTableData = currentProject.tables.filter(t => selectedTables.has(t.id));
        const exportData = {
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex items-center space-x-2">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-600">{project.tables.length} tables</p>
                                    </div>
                                </div>
                                <button
                                    onClick={(e) => {
            ...
            <div className="flex-1 p-6 overflow-y-auto">
                {currentProject ? (
                    <div>
                        ...
                        <div className="space-y-4">
                            {currentProject.tables.length > 0 && (
                                <div className="flex items-center justify-between mb-4">
                                    <div className="flex items-center space-x-2">
                                        <input
                                            type="checkbox"
                                            checked={selectedTables.size === currentProject.tables.length && currentProject.tables.length > 0}
                            ...
                            {currentProject.tables.map((table) => (
                                <div key={table.id} className="border border-gray-300 rounded-lg">
                                    <div className="flex items-center justify-between p-4 bg-gray-50 border-b border-gray-300">
                                        <div className="flex items-center space-x-2">
                                            ...
                                            <span className="text-sm text-gray-500">({table.data.length} rows)</span>
                                        </div>
                                        <div className="flex items-center space-x-2">
                                            <button
                                                onClick={() => setShowTablePreview(showTablePreview === table.id ? null : table.id)}
                                                className="p-1 text-gray-500 hover:text-gray-700"
                                            >
                                                {showTablePreview === table.id ? <EyeOff size={16} /> : <Eye size={16} />}
                                            </button>
                                            <button
                            ...
                            {currentProject.tables.length === 0 && (
                                <div className="text-center py-12 text-gray-500">
                                    ...
                                    <p className="text-sm">Add tables by processing images or importing Excel files.</p>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="text-center py-12 text-gray-500">
                        ...
                        <p className="text-sm">Create a new project or select an existing one.</p>
                    </div>
                )}
            </div>
        </div>
