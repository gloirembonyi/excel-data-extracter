 ...
 */
export const exportToExcel = (data: Record<string, any>[], fileName: string = 'exported_data.xlsx', sheetName: string = 'Data') => {
    if (data.length === 0) {
 ...
 */
export const exportToCSV = (data: Record<string, any>[], fileName: string = 'exported_data.csv') => {
    if (data.length === 0) {
        throw new Error('No data to export');
    }
...
export const compareDatasets = (
    ...
    if (keyField) {
        ...
        const map1 = new Map(data1.map((item, index) => [item[keyField], { ...item, _index: index } as Record<string, any>]));
        const map2 = new Map(data2.map((item, index) => [item[keyField], { ...item, _index: index } as Record<string, any>]));
        ...
        for (const [key, item1] of map1) {
            ...
            if (item2) {
                ...
                for (const field of fields) {
                    if (field !== keyField && field !== '_index' && item1[field] !== item2[field]) {
