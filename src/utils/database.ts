...
export class DatabaseManager {
  ...
  async addMasterData(
    projectId: string,
    data: Omit<MasterDataItem, "id" | "created_at" | "updated_at">[]
  ): Promise<void> {
    const response = await fetch(
      `${this.apiUrl}/projects/${projectId}/master-data`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async updateMasterData(
    ...
    const response = await fetch(
      `${this.apiUrl}/projects/${projectId}/master-data/${itemId}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  async findMatches(
    ...
    const response = await fetch(
      `${this.apiUrl}/projects/${projectId}/match-data`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async exportToExcel(
    projectId: string,
    format: "screen_cpu" | "unified" = "screen_cpu"
  ): Promise<Blob> {
    const response = await fetch(
      `${this.apiUrl}/projects/${projectId}/export-excel?format=${format}`
    );
  ...
  async validateData(
    ...
    const response = await fetch(
      `${this.apiUrl}/projects/${projectId}/validate-data`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ data }),
...
export const createProject = (name: string, description?: string) =>
  dbManager.createProject(name, description);
export const getProjects = () => dbManager.getProjects();
export const getProject = (projectId: string) =>
  dbManager.getProject(projectId);
export const addMasterData = (
  projectId: string,
  data: Omit<MasterDataItem, "id" | "created_at" | "updated_at">[]
) => dbManager.addMasterData(projectId, data);
export const getMasterData = (projectId: string) =>
  dbManager.getMasterData(projectId);
export const findMatches = (projectId: string, extractedData: any[]) =>
  dbManager.findMatches(projectId, extractedData);
export const exportToExcel = (
  projectId: string,
  format?: "screen_cpu" | "unified"
) => dbManager.exportToExcel(projectId, format);
