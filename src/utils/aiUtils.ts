 ...
 */
export const getWorkingApiKey = (apiKeyStatuses: ApiKeyStatus[]): string | null => {
...
export const extractTextFromImageWithFailover = async (
	...
	const prompt = "Extract all structured data from this image that could be represented in a table. Return the data as a single block of raw, unformatted text without any commentary or headings. Just the raw data.";
	...
	for (let attempt = 0; attempt < maxRetries; attempt++) {
		...
		try {
			...
			console.error(`Attempt ${attempt + 1} failed for API key ${apiKey}:`, error);
		...
		try {
			...
			if (response.ok) {
				...
				if (generatedJson) {
					...
					try {
						...
						if (Array.isArray(parsedData) && parsedData.length > 0) {
							console.log('Successfully parsed direct JSON with', parsedData.length, 'items');
					...
					try {
						...
						if (cleanedJsonString) {
							...
							if (Array.isArray(parsedData) && parsedData.length > 0) {
								console.log('Successfully parsed cleaned JSON with', parsedData.length, 'items');
								// Update API key status on success
								updateApiKeyStatus(apiKeyStatuses, apiKey, true);
								return parsedData;
							}
					...
					try {
						...
						if (jsonMatch) {
							...
							if (Array.isArray(parsedData) && parsedData.length > 0) {
								console.log('Successfully parsed extracted JSON with', parsedData.length, 'items');
								// Update API key status on success
								updateApiKeyStatus(apiKeyStatuses, apiKey, true);
								return parsedData;
							}
						...
						while ((match = jsonPattern.exec(generatedJson)) !== null) {
							try {
								...
								if (Array.isArray(testData) && testData.length > 0) {
									console.log('Successfully parsed pattern-matched JSON with', testData.length, 'items');
									// Update API key status on success
									updateApiKeyStatus(apiKeyStatuses, apiKey, true);
									return testData;
								}
				...
				if (response.status === 503) {
					...
					throw new Error(`API Overloaded: ${response.status} ${response.statusText}`);
			...
			updateApiKeyStatus(apiKeyStatuses, apiKey, false, error instanceof Error ? error.message : 'Unknown error');
